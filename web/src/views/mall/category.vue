    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="所属模块" prop="moduleId">
            <el-select v-model="dataForm.moduleId" >
             <el-option v-for="item in moduleList" :key="item.name" :label="item.name" :value="item.id"/>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    getModuleList(){
        getAll("Module/list",this.listQuery).then(response=>{
        if(response.data.errno==0){
             this.moduleList = response.data.data.items
      ...
      listCategory(this.listQuery)
        .then(response => {
          ...
          this.list.forEach(item=>{
            this.moduleList.forEach(items=>{
              if(item.moduleId==items.id){
              item.moduleName = items.name
    ...
    uploadPicUrl: function(response) {
      this.dataForm.picUrl =  response.data.url.replace("http://localhost:8080/wx/storage/fetch",this.filePath)
    },
    createData() {
