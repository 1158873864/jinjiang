      ...
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        <el-form-item label="问题" prop="question">
          ...
          <el-select v-model="dataForm.level" @change="onLevelChange">
            <el-option label="一级问题" value="L1"/>
            <el-option label="二级问题" value="L2"/>
          </el-select>
        </el-form-item>
            <el-form-item v-if="dataForm.level === 'L2'" label="一级问题" prop="pid">
          <el-select v-model="dataForm.pid">
            <el-option v-for="item in catL1" :key="item.id" :label="item.question" :value="item.id"/>
          </el-select>
        </el-form-item>
        <el-form-item label="回复" v-if="dataForm.level === 'L2'"  prop="answer">
...
export default {
  ...
  methods: {
   ...
   getPidList(){
     ...
     listQuery.limit=50
     listQuery.page= 1
       listIssue(listQuery).then(response => {
        this.catL1 = response.data.data.items
