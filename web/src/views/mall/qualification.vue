<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="资质名称" prop="name"/>

      <el-table-column align="center" label="资质类型" prop="type"/>
      ...
      <el-table-column align="center" label="操作" width="200" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['POST /admin/category/delete']" type="danger" size="mini" @click="handleDelete(scope.row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        name: [{ required: true, message: '资质名不能为空', trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          postData("qualification/createByAdmin",this.dataForm).then(response=>{
  ...
  handleDelete(id) {
    deleteByID("qualification/delete?id="+id).then(response=>{
      //删除一条成功
