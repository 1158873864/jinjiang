      ...
      <el-form ref="dataForm" :rules="rules" :model="dataForm" status-icon label-position="left" label-width="100px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="广告位置" prop="position">
          <el-select v-model="dataForm.position" placeholder="请选择">
            ...
            <el-option :value="2" label="移动端上区域"/>
            <el-option :value="3" label="移动端下区域"/>
            <el-option :value="4" label="页眉"/>
            <el-option :value="5" label="轮播图广告位"/>
            <el-option :value="6" label="顶部右侧广告位"/>
          </el-select>
        </el-form-item>
...
export default {
  ...
  created() {
    ...
    this.getList()
    console.log(this.shopId,77788, this.dataForm)
  },
  methods: {
    getList() {
    ...
    uploadUrl: function(response) {
      ...
      this.dataForm.url = response.data.url.replace("http://localhost:8080/wx/storage/fetch",this.filePath);
    },
    createData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          console.log(this.dataForm,74445)
