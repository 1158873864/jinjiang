<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" size="small" element-loading-text="正在查询中。。。" border fit highlight-current-row>
      ...
      <el-table-column align="center" label="用户id" prop="user"/>

      <el-table-column align="center" label="优惠券id" prop="discount"/>
...
export default {
  ...
  getList() {
    ...
    axios({
      method: 'get',
      url: config.baseApi + "user/find/identity?identity=member&page="+ (this.listQuery.page-1)+"&size=20",
      headers:{
        "X-Litemall-Admin-Token":sessionStorage.getItem('token')
      }
    }).then(response => {
    ...
    handleFilter() {
      ...
      axios({
        method: 'get',
        url: config.baseApi + "user/find/identity/query?identity=member&query="+this.listQuery.key+"&page="+ (this.listQuery.page-1)+"&size=20",
        headers:{
          "X-Litemall-Admin-Token":sessionStorage.getItem('token')
        }
      }).then(response => {
